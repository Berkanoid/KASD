...
class SmartString
{
    ...
    public SmartString(string s)
    {
        ...
        lengths = words.Select(w => w.Length).OrderBy(len => len).ToList();
    }
    public override bool Equals(object obj)
    {
        if (obj is not SmartString other) return false;
        if (lengths.Count != other.lengths.Count) return false;
    ...
    {
        int hash = 17;
        foreach (int len in lengths) hash = hash * 31 + len;
        return hash;
    }
...
class Program
{
    static void Main()
    {
        ...
        MyHashSet<SmartString> set = new MyHashSet<SmartString>();
        foreach (string line in lines)
        {
            if (string.IsNullOrWhiteSpace(line)) continue;
            set.Add(new SmartString(line));
        }
        Console.WriteLine("Количество уникальных строк (по правилу сравнения): " + set.Size());
        Console.WriteLine("Уникальные строки:");
        foreach (SmartString ss in set.GetAll())
